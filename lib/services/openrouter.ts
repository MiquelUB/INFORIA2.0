...
export class OpenRouterService {
  ...
  async compileReportInfo(data: {
    ...
    patientData: {
      name?: string; // Opcional, ya no se usa en el prompt
      alias: string; // OBLIGATORIO: Ej. "Mujer de 42 años"
      age?: number;
...
⛔ PROTOCOLO DE ANONIMIZACIÓN TOTAL (PRIORIDAD MÁXIMA):
1. BAJO NINGUNA CIRCUNSTANCIA incluyas nombres propios en el reporte.
2. Si detectas un nombre en el contexto (notas, historial, transcripción), CENSÚRALO y REEMPLÁZALO automáticamente por el alias.
3. El incumplimiento de esta norma es un fallo crítico de seguridad.
4. Refiérete al paciente SIEMPRE Y ÚNICAMENTE como: "${data.patientData.alias}".
