...
export function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading) {
      const isPublicPage = ['/login', '/activate', '/pricing', '/account', '/auth/callback', '/auth/auth-code-error'].some(path => pathname?.startsWith(path));
  ...
  if (!user) {
     const isPublicPage = ['/login', '/activate', '/pricing', '/account', '/auth/callback'].some(path => pathname?.startsWith(path));
