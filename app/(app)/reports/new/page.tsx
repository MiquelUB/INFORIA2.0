...
function ReportWorkspaceContent() {
  ...
  const handleSaveReport = () => {
    ...
    startSaving(async () => {
      const result = await saveReportToGoogleDrive(fileName, reportDraft, "text/plain", patientId || undefined);
      if (result.success) {
        toast({ title: "Informe guardado", description: `Archivo ID: ${result.data} (Zero-Knowledge)` });
      } else {
