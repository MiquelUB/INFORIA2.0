...
export default function MyAccountClient() {
  ...
  const { data: profile, isLoading: profileLoading, refetch } = useUserProfile(user?.id || null); // Change mutate to refetch
    ...
    const checkStripeSession = async (sessionId: string) => {
      ...
      try {
        ...
        if (sessionData.status === 'paid' || sessionData.status === 'complete') {
          ...
          // 'mutate' es de SWR/React-Query, ajusta según tu hook
          if (refetch) refetch(); // Change mutate to refetch
  ...
  }, [searchParams, router, refetch]); // Añadimos 'refetch' a las dependencias
